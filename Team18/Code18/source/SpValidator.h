...
namespace spa {
    ...
    public:
        ...
        //struct std::exception : std::exception {
        //    std::exception(const std::string& m);
        //};
    private:
        std::unordered_set<TokenType> condExprToken = { TOKEN_BOOL_AND, TOKEN_BOOL_OR };
        std::unordered_set<TokenType> relExprToken = { TOKEN_COND_EQ, TOKEN_COND_NEQ, TOKEN_COND_LT, TOKEN_COND_LTE, TOKEN_COND_GT, TOKEN_COND_GTE };
        ...
        std::unordered_set<TokenType> nameToken = { TOKEN_NAME, TOKEN_IF, TOKEN_WHILE, TOKEN_PRINT, TOKEN_READ };
